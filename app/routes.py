...
def generate():
    ...
    if request.method == "GET":
        # read courses information file to form autocomplete list
        with open("course_info.json", "r") as f:
            course_list = json.loads(f.read())
            result = {}
            for e in course_list:
                result[e] = None
        # read credential file for connecting AWS Comprehend server
    ...
    if request.method == "POST":
        ...
        connect("course_info")
        try:
            # access to AWS Comprehend and get response
            ...
            # remove duplicated and useless words from provided key phrases
            result = set()
            pattern = re.compile("[a-z]{4}\d{4}")
            for e in cc["ResultList"][0]["KeyPhrases"]:
                e = e["Text"]
                temp = e.lower()
                if temp.find("student") != -1 or temp.find("course") != -1 or temp.find("unsw")!=-1 or temp.find("study")!=-1 or pattern.findall(temp):
                    continue
...
def generate2():
    ...
    if request.method == "POST":
        try:
            # obtain user's selection of verbs and key phrases
...
def generate3():
    ...
    if request.method == "POST":
        try:
            # obtain learning outcomes generated by user
            outcome_list = []
            response = request.get_json()
...
def evaluate():
    ...
    if request.method == "POST":
        try:
            # analyse the quality of existing learning outcomes indicated by user
            ...
            # check how many words are not measurable verbs
